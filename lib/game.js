...
const BOT_STRATEGY_SET_NOVICE = server_constants.constants.BOT_STRATEGY_SET_NOVICE;
...
Game = function(game_id, client_id, version, istest, player1, player2, game_rounds) {
    ...
    this.player1_points_total = 0;
    this.player2_points_total = 0;
...
Round = function(game) {
    ...
    this.round_index = game.current_round_index;
    this.player1_move = null;
    this.player2_move = null;
    this.player1_outcome = null;
    this.player2_outcome = null;
...
BotAgent = function(ability) {
    ...
    if (ability.includes("novice")) {
        this.fixed_strategy = _.sample(BOT_STRATEGY_SET_NOVICE)
    }
...
SimulatedLearningAgent.prototype.chooseMove = function(game) {
    var current_ability = this.initial_ability + (this.final_ability - this.initial_ability) * (game.current_round_index / this.game_rounds)
    console.log("game.js:\t ", current_ability)
    if (weighted.select({"oracle": current_ability, "random": 1 - current_ability}) == "oracle") {
        return this.oracle_strategy_transitions[game.current_round.player2_move]
    }
    else {
        return weighted.select(BOT_RANDOM_MOVES);
    }
...
GameHandler.prototype.getCurrentGame = function(client) {
    for (game_id in this.active_games) {
        ...
        console.log(current_game.client_id, client.userid)
...
// Function to create a new game and add this client as the first player
GameHandler.prototype.createGame = function(client, version, istest) {
    console.log("game.js:\t creating new game for client: ", client.userid, "\t version: ", version);
    if version == "1":
        opponent = new BotAgent("novice")
        agent = new SimulatedLearningAgent(0, 0.66, GAME_ROUNDS)
    if version == "2":
        opponent = new BotAgent("novice")
        agent = new SimulatedLearningAgent(0.66, 0.66, GAME_ROUNDS)
    if version == "3":
    ...
    newgame_id = UUID()
    var newgame = new Game(id=newgame_id, client_id=client.userid, version=version, istest=istest, player1=agent, player2=opponent, total_rounds=GAME_ROUNDS)
    // Add game to game server directory of games in play
    this.active_games[newgame_id] = newgame;
    // Update client telling them they're waiting and giving them the game id
    client.emit("newgame", {game_id: newgame_id});
...
GameHandler.prototype.nextRound = function(client, data) {
    ...
    console.log("game.js:\t currentRound", current_round);
    current_game.current_round = current_round;
    current_game.player1_points_total += current_round.player1_points; // update game total points
    current_game.player2_points_total += current_round.player2_points; // update game total points
    // Update both clients (or single client if playing against a bot)
...
//         }
//         // If this was an unpaired bot game, log additional info about the bot strategy
//         if (current_game.version == "4" && current_game.player2.client_id == "bot") {
//             data_obj["player2_botid"] = current_game.player2.uid;
//             data_obj["player2_bot_strategy"] = current_game.player2.fixed_strategy;
//             data_obj["player2_bot_move_probabilities"] = current_game.player2.fixed_strategy_transitions;
//         }
