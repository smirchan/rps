...
SimulatedLearningAgent = function(initial_ability, final_ability, game_rounds) {
    ...
    this.antioracle_strategy_transitions = TRANSITION_MOVE_MAP["-"]
}
...
SimulatedLearningAgent.prototype.chooseMove = function(game, current_ability) {
    if (current_ability >= 1/3) {
        var prob_oracle = (current_ability - 1/3) / (BOT_FIXED_MOVE_PROB - 1/3)
        if (weighted.select({"oracle": prob_oracle, "random": 1 - prob_oracle}) == "oracle") {
        ...
        var prob_antioracle = (1/3 - current_ability) / (1/3 - (1 - BOT_FIXED_MOVE_PROB)/2)
        if (weighted.select({"antioracle": prob_antioracle, "random": 1 - prob_antioracle}) == "antioracle") {
            return this.antioracle_strategy_transitions[game.current_round.denoised_player2_move]
        }
        else {
            return weighted.select(BOT_RANDOM_MOVES);
        }
...
SimulatedLearningAgent.prototype.makeMove = function(game) {
    // Step-wise increase
    var num_steps = 4;
    var segment = Math.floor((game.current_round_index - 1) / (this.game_rounds / num_steps));
    var ability = this.initial_ability + segment * (this.final_ability - this.initial_ability) / (num_steps - 1);
    ...
    console.log("game.js:\t received agent move: ", "(ability: ", ability, ")");
...
GameHandler.prototype.newGame = function(client, versions, istest, index) {
    ...
    if (version == "1") {
        ...
        agent = new SimulatedLearningAgent(1/3, 1/3, GAME_ROUNDS)
    }
    if (version == "2") {
        opponent = new BotAgent("novice-same")
        agent = new SimulatedLearningAgent(1/3, 2/3-1/20, GAME_ROUNDS)
    }
    if (version == "3") {
        opponent = new BotAgent("novice-same")
        agent = new SimulatedLearningAgent(2/3-1/20, 2/3-1/20, GAME_ROUNDS)
    }
    if (version == "4") {
    ...
    if (version == "10") {
        opponent = new BotAgent("novice-sequence")
        agent = new SimulatedLearningAgent(0.2, 0.2, GAME_ROUNDS)
    }
    if (version == "11") {
        opponent = new BotAgent("novice-sequence")
        agent = new SimulatedLearningAgent(0.2, 0.8, GAME_ROUNDS)
    }
    if (version == "12") {
        opponent = new BotAgent("novice-sequence")
        agent = new SimulatedLearningAgent(0.8, 0.8, GAME_ROUNDS)
    }
    if (version == "13") {
        opponent = new BotAgent("expert")
        agent = new SimulatedLearningAgent(0.2, 0.2, GAME_ROUNDS)
...
GameHandler.prototype.getPilotVersions = function() {
        opponents = {10: 1/2, // novice-sequence
                     13: 1/2  // expert
                    }
...
GameHandler.prototype.nextRound = function(client, data) {
    ...
    // If last round of game or experiment, notify client, else begin next round
