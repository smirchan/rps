...
start_instructions = function() {
    ...
    inst = new Instructions(HTML_LOOKUP["trust_instructions"], INSTRUCTION_ARRAY, connect_to_server);
    inst.run();
};
...
initialize_game = function(game) {
    ...
    if (urlParams.has("ver") && urlParams.get("ver").includes("4")) {
        game.version = 4;
    }
    if (urlParams.has("ver") && urlParams.get("ver").includes("5")) {
        game.version = 5;
    }
    if (urlParams.has("ver") && urlParams.get("ver").includes("6")) {
        game.version = 6;
    }
    if (urlParams.has("ver") && urlParams.get("ver").includes("7")) {
        game.version = 7;
    }
    if (urlParams.has("ver") && urlParams.get("ver").includes("8")) {
        game.version = 8;
    }
    console.log("client.js:\t initializing game. \n\tTEST: ", game.istest, "\n\tVERSION: ", game.version);
    $("body").load(HTML_LOOKUP["trust_experiment"])
}
...
client_newgame = function(data) {
    ...
    show_next_button(request_next_round.bind(this));
}
...
display_results = function(data) {
    ...
    show_thinking(finish_displaying_results, data);
};
...
show_thinking = function(callback, data) {
    ...
    var check = function(){
        if(countdown == 1){
            console.log("done_thinking")
            callback(data);
        }
        else {
            countdown = countdown - 1;
            set_countdown(countdown);
            setTimeout(check, 700); // check again in a second
        }
    }
    check();
}
...
finish_displaying_results = function(data) {
    ...
    if (agent_outcome == "win") {
        outcome_text = "Agent 1 won this round!";
    } else if (agent_outcome == "loss") {
        outcome_text = "Agent 1 lost this round.";
...
display_survey = function(data) {
    ...
    $("body").load(HTML_LOOKUP["trust_survey"], function() {
        display_message("Evaluate Agent 1")
        show_next_button(callback)
    })
}
